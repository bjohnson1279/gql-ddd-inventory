...
describe('GetStockValuationReportUseCase', () => {
  ...
  beforeEach(() => {
    ...
    calculateCostSpy = jest.spyOn(CostLayerService.prototype, 'calculateCostBatch');
  });
  ...
  it('should calculate cost breakdown properly and accumulate total value', async () => {
    ...
    calculateCostSpy.mockResolvedValueOnce([new CostBreakdown(10, 1000), new CostBreakdown(5, 500)]);
  ...
  it('should include 0 value line item when cost calculation throws exception', async () => {
    ...
    calculateCostSpy.mockResolvedValueOnce([null]);
  ...
  it('should ignore inventory items with quantity <= 0', async () => {
    ...
    mockInventoryRepo.findAll.mockResolvedValue([invItem1]);
