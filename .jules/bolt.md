...
## 2024-06-03 - Redundant Array.find in Sequential Extractions
**Learning:** Performing sequential `.find()` calls on the same array to extract multiple independent elements (e.g., `attrs.find(a => a.name === '...')`) scales linearly O(N*M) with the number of attributes.
**Action:** Always pre-compute a `Map` in a single pass O(N) over the array before performing multiple O(1) extractions.
