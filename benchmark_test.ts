...
async function run() {
  const code = fs.readFileSync('src/infrastructure/persistence/PostgresInventoryRepository.ts', 'utf8');
  if (code.includes('await Promise.all')) {
    console.log("Currently OPTIMIZED");
  } else {
    console.log("Currently UNOPTIMIZED");
  }
  ...
  const prisma: any = {
    inventoryItem: {
      findMany: async () => {
        // Return first 25 as existing to trigger both paths
        return Array.from({length: 25}, (_, i) => ({ id: `id-${i}` }));
      }
    },
